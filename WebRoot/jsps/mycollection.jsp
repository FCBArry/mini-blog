...
<body id="top">
	...
	<div class="container">
		...
		<div class="header row">
			<h3 class="lead">${sessionScope.sessionUser.username }</h3>
				...
				<ul class="nav navbar-nav navbar-right">
					<li class="active"><a
						href="<c:url value='/info_myBlog.action?uid=${sessionScope.sessionUser.uid }'/>"><span
							class="glyphicon glyphicon-book"></span> 博客</a>
					</li>
					<li><a href="<c:url value='/info_myCollection.action'/>"><span
							class="glyphicon glyphicon-heart"></span> 收藏</a>
					</li>
				</ul>
			</div>
			</nav>
		...
		<div class="row main">
								...
								<div class="panel-body table-responsive">
									<table class="table table-condensed table-striped">
										<tbody>
											...
											<tr>
												<td><span class="glyphicon glyphicon-qrcode"></span>
												</td>
												<td>${info.iqq }</td>
											</tr>
											<tr>
											...
											<tr>
												<td><span class="glyphicon glyphicon-tower"></span>
												</td>
												<td>${info.icompany }</td>
											</tr>
										</tbody>
			...
			<div class="col-sm-9 right">
				...
				<c:forEach items="${pb.beanList }" var="collection">
					<div class="content col-sm-12">
						<div class="row">
							<div class="col-sm-2">
								<h6>
									<span class="label label-info"><span
										class="glyphicon glyphicon-thumbs-up"></span>${collection.post.pzan
										}+</span>
								</h6>
							</div>
							<div class="col-sm-6">
								<a href="<c:url value='/post_findByPid.action?pid=${collection.post.pid }'/>"><h6 class="lead">${collection.post.ptitle }</h6>
								</a>
							</div>
							<div class="col-sm-2">
								<h6>
									<span class="label label-primary">${collection.post.pcomment
										}+</span>
								</h6>
							</div>
							<div class="col-sm-2">
								<h6>
									<span class="label label-success"><span
										class="glyphicon glyphicon-user"></span>${collection.post.user.username }</span>
								</h6>
							</div>
						</div>
						<div class="row">
							<div class="col-sm-6 col-sm-offset-2">
								<p>${collection.post.pcontent }</p>
							</div>
							<div class="col-sm-4">
								<a
									href="<c:url value='/post_findByPid.action?pid=${collection.post.pid }'/>">
									<p>精彩弹幕...</p>
								</a>
							</div>
						</div>
				...
				<div class="page row">
					...
					<ul class="pagination pagination-md">
						<c:choose>
							...
							<c:otherwise>
								<li><a href="${pb.url }&pc=${pb.pc-1}">&laquo;</a>
								</li>
							</c:otherwise>
						</c:choose>
						<c:choose>
						...
						<c:choose>
							...
							<c:otherwise>
								<li><a href="${pb.url }&pc=${pb.pc+1}">&raquo;</a>
								</li>
							</c:otherwise>
						</c:choose>
					</ul>
